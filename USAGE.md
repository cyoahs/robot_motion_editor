...
1. 加载 URDF 与 CSV 后，展开侧栏 **IK 编辑**。
2. 勾选 **启用末端拖拽 IK**，在 **末端 Link** 下拉框选择连杆（可用 `left_hand` / `left_foot` 等快捷按钮，适配 G1 命名）。
3. 在 3D 视口中拖拽末端手柄；松手后**自动**在当前帧写入关键帧（残差与滑块编辑一致）。
4. 腿链可勾选 **锁定足底高度**（保持拖拽开始时足底世界高度）。
5. 播放时间轴时 IK 手柄自动隐藏。
